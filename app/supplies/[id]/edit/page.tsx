...
export default function EditSupplyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: error.message || '更新失败，请重试' });
    } finally {
  ...
  const handleDelete = async () => {
    if (!confirm('确定要删除这条耗材记录吗？此操作不可恢复。')) {
      return;
    }
      ...
      <div className={layout.page}>
        <div className="flex justify-center items-center min-h-screen">
          <div className="text-center">
            <svg className="animate-spin h-12 w-12 mx-auto text-apple-blue" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
