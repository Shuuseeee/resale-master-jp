-- Migration: Add User Authentication and RLS Policies
-- Description: Add user_id fields to all tables and implement Row Level Security
...
-- Add user_id to products table (if exists)
ALTER TABLE products
ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Add user_id to sales table (if exists)
ALTER TABLE sales
ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
-- ============================================
-- 9. Create RLS Policies for sales
-- ============================================
...
-- ============================================
-- 10. Create function to automatically set user_id
-- ============================================
...
-- ============================================
-- 11. Update views to include user_id filtering
-- ============================================
...
-- Enable RLS on views
ALTER VIEW financial_water_level SET (security_invoker = true);
ALTER VIEW upcoming_payments SET (security_invoker = true);
ALTER VIEW pending_points SET (security_invoker = true);
...
COMMENT ON COLUMN products.user_id IS 'User ID from auth.users - ensures data isolation';
COMMENT ON COLUMN sales.user_id IS 'User ID from auth.users - ensures data isolation';
